...
def add_neat_bev(bev_transform, left_bev_transform,
                 right_bev_transform, vehicle_id_stream,
                 release_sensor_stream, pose_stream,
                 gnss_stream, imu_stream, goal_location, time_to_decision_stream):

    bev_camera_setup = RGBCameraSetup('bev_rgb_camera', 400, 300, bev_transform, 100)
    (bev_camera_stream,
     _) = pylot.operator_creator.add_camera_driver(bev_camera_setup,
                                                   vehicle_id_stream,
                                                   release_sensor_stream)

    left_bev_camera_setup = RGBCameraSetup('left_bev_rgb_camera', 400, 300, left_bev_transform, 100)
    (left_bev_camera_stream,
     _) = pylot.operator_creator.add_camera_driver(left_bev_camera_setup,
                                                   vehicle_id_stream,
                                                   release_sensor_stream)

    right_bev_camera_setup = RGBCameraSetup('right_bev_rgb_camera', 400, 300, right_bev_transform, 100)
    (right_bev_camera_stream,
     _) = pylot.operator_creator.add_camera_driver(right_bev_camera_setup,
                                                   vehicle_id_stream,
                                                   release_sensor_stream)

    from pylot.perception.neat_bev.neat_bev_operator import NeatBEVOperator
    op_config = erdos.OperatorConfig(name='neat_bev_operator',
                                     flow_watermarks=False,
                                     log_file_name=FLAGS.log_file_name,
                                     csv_log_file_name=FLAGS.csv_log_file_name,
                                     profile_file_name=FLAGS.profile_file_name)
    [ control_stream
     ] = erdos.connect(NeatBEVOperator, op_config,
                       [bev_camera_stream, left_bev_camera_stream,
                        right_bev_camera_stream, pose_stream,
                        gnss_stream, imu_stream, time_to_decision_stream],
                       goal_location, FLAGS)
