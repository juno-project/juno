...
def compute_person_speed_factor(ego_location_2d, person_location_2d, wp_vector,
                                person_angle_hit_zone, person_distance_hit_zone, coast_factor, 
                                person_angle_emergency_zone, person_distance_emergency_zone) -> float:
    speed_factor_p = 1
    ...
    print('Person vector {}; dist {}; angle {}'.format(
    ...
    # is in the hit zone.
    if (math.fabs(p_angle) < person_angle_hit_zone
            and p_dist < person_distance_hit_zone):
        # Person is in the hit zone.
        speed_factor_p = min(
            speed_factor_p,
            p_dist / (coast_factor * person_distance_hit_zone))
    if (math.fabs(p_angle) < person_angle_emergency_zone
            and p_dist < person_distance_emergency_zone):
