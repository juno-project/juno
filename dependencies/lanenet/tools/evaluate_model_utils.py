...
def calculate_model_precision(input_tensor, label_tensor):
    ...
    final_output = tf.expand_dims(tf.argmax(input=logits, axis=-1), axis=-1)
    ...
    accuracy = tf.divide(
        accuracy,
        tf.cast(tf.shape(input=tf.gather_nd(label_tensor, tf.compat.v1.where(tf.equal(label_tensor, 1))))[0], tf.int64))
...
def calculate_model_fp(input_tensor, label_tensor):
    ...
    false_pred = tf.cast(tf.shape(input=pix_cls_ret)[0], tf.int64) - tf.math.count_nonzero(
        tf.gather_nd(label_tensor, idx)
    )

    return tf.divide(false_pred, tf.cast(tf.shape(input=pix_cls_ret)[0], tf.int64))
...
def calculate_model_fn(input_tensor, label_tensor):
    ...
    mis_pred = tf.cast(tf.shape(input=label_cls_ret)[0], tf.int64) - tf.math.count_nonzero(pix_cls_ret)

    return tf.divide(mis_pred, tf.cast(tf.shape(input=label_cls_ret)[0], tf.int64))
...
def get_image_summary(img):
    ...
    image = img - tf.reduce_min(input_tensor=img)
    image /= tf.reduce_max(input_tensor=img) - tf.reduce_min(input_tensor=img)
    image *= 255
